...
#'
#' Using morphology functions \code{\url{https://en.wikipedia.org/wiki/Mathematical_morphology}} to get rid of unwanted noise and features.
#' With this package the features to eliminate will mostly be random weeds and bushy leaves. The specific functions being use are opening (\code{\url{https://en.wikipedia.org/wiki/Opening_(morphology)}) and closing (\code{\url{https://en.wikipedia.org/wiki/Closing_(morphology)}).
...
#'
#' @param size A number, the starting size of the kernel for morphology (minimum of 2).
#'
#' @example
#' \donttest{
#' final_blob <- blobify(images, size)
#' }
#'
#' @export
