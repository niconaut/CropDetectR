...
```{r}
img <- load.image('https://raw.githubusercontent.com/niconaut/CropDetectR/master/images/before_rows.JPG')
```

Loading the image using the *imager* loading function to prepare it for the ExG process.
...
Using the Otsu method of image thresholding we are able to capture only the plant material as white (value of 1) and the rest of the non-organic material is black (value of 0). This will be very benficial for the next steps because a binary image is needed.
...
Performing simple image rotations will allow us to find out the best degree of rotation to get the crop rows vertical. In this example we are rotating the image every 45 degrees to check if there is a better angle other than its base. It is important that they are vertical for the next few steps which will be explained in the next section.
...
Making the images smoother is necessary when we are trying to find true local minimas and maximas. If the image is very sparatic and jagged identifying the locals will be a tough challenge since there is a lot of up and down change. As we see in the smoothed image, there are only a few locals and in the next steps we can use a ratio to ignore the smaller maximas and minimas, leaving us with only the very pronounced ones.
